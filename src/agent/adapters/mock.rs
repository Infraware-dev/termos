...
use crate::agent::adapters::mock::workflow::Playbook;
pub use crate::agent::adapters::mock::workflow::Workflow;
use crate::agent::error::AgentError;
use crate::agent::traits::{Agent, EventStream};
use crate::agent::types::{HealthStatus, ResumeResponse};
use crate::agent::{AgentEvent, Interrupt, Message, MessageRole, RunInput, ThreadId};
...
impl MockAgent {
    ...
    fn build_step_interrupt_events(
        ...
        step_index: usize,
        run_id: &str,
    ) -> (Vec<Result<AgentEvent, AgentError>>, WorkflowState) {
        let phase = &playbook.phases[phase_index];
    ...
    fn build_step_completion_events(
        state: &WorkflowState,
        run_id: &str,
    ) -> (Vec<Result<AgentEvent, AgentError>>, Option<WorkflowState>) {
...
impl Agent for MockAgent {
    ...
    async fn stream_run(
        ...
        let messages = threads
            .get_mut(&thread_id.0)
            .ok_or_else(|| AgentError::thread_not_found(&thread_id.0))?;
    ...
    async fn resume_run(
        ...
        if !self.threads.read().await.contains_key(&thread_id.0) {
            return Err(AgentError::thread_not_found(&thread_id.0));
        }
...
mod tests {
    ...
    async fn test_stream_run() {
        ...
        let mut stream = agent.stream_run(&thread_id, input).await.unwrap();
    ...
    async fn test_thread_not_found() {
        ...
        let result = agent.stream_run(&fake_thread, input).await;
