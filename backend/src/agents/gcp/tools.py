"""GCP-related tools and utilities.

This module initializes GCP MCP (Model Context Protocol) tools at import time.
...
def _initialize_mcp_tools():
    ...
    Returns:
        list: List of LangChain-compatible tool objects from the GCP MCP server.
    ...
    """
    # Configure the GCP MCP server connection
    # Uses npx to run the @modelcontextprotocol/server-gcp package
...
try:
    ...
    logger.info("Successfully initialized %d GCP MCP tools", len(mcp_tools))
    ...
    logger.warning("Failed to initialize GCP MCP tools: %s", e)
    mcp_tools = []
